...
with open('./sigma.p', 'rb') as f:
    sigma = pickle.load(f)

with open('./Vt.p', 'rb') as f:
    Vt = pickle.load(f)
...
def rmse(trainData,testData):
    ...
    for i in range(testData.shape[0]):
        for j in range(testData.shape[1]):
            if trainData[i][j]>0 and trainData[i][j]<5:
...
print ("RMSE SCORE IS:",rmse(A_new,testData))
print ("PRECISION ON TOP K IS:",top_k_precision(A_new,testData))
print ("Spearman's Rank Correlation",spearman_rank_correlation(A_new,testData))
